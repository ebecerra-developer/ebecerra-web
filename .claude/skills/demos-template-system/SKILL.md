...
- `tienda` — supermercado online **ecommerce MULTIPÁGINA** (**La Cesta**, slug `la-cesta`). Rompe el patrón apilado (como `expedicion`): tiene rutas propias (categoría/producto/buscar/carrito/checkout), cesta persistente, y un contrato de comercio con forma de Medusa. **Antes de tocarla o de crear otra tienda, lee `/demos-tienda-ecommerce`** (contrato/adaptador, arquitectura multipágina, pipeline de imágenes IA y gotchas caros). **GOTCHA que aplica a toda plantilla multipágina:** el wrapper `data-template="<plantilla>"` va en el **chrome compartido** (no solo en la page raíz), o toda la demo hereda los tokens `:root` de fisio y sale sin contraste.
