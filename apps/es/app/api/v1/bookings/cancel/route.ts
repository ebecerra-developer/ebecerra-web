import { consumeToken } from "@ebecerra/bookings/tokens";
...
/**
 * GET /api/v1/bookings/cancel?token=<signed>
 *
 * Landing page tras click en el email de "cancelar mi cita".
...
export async function GET(request: Request): Promise<Response> {
  ...
  const { data: booking, error } = await supabase
    ...
    .select(
      "id, slot_start_utc, contact_name, locale, booking_tenant_id, service_id"
    )
    .maybeSingle();
  if (error) throw error;
  if (!booking) {
    return htmlError(409, "Esta cita ya estaba cancelada o completada.");
  }
  ...
  try {
    await sendCancelledEmail({ bookingId: booking.id });
  } catch (e) {
    console.error("[bookings/cancel] sendCancelledEmail failed:", e);
  }
  ...
  return html(`
    ...
    <p>Tu cita con <strong>${escape(tenantName)}</strong> del <strong>${escape(slotLocal)}</strong> ha sido cancelada.</p>
    <p>Si quieres reservar otra, vuelve a la web.</p>
  `);
}
...
function reasonToMessage(reason: string): string {
  switch (reason) {
    case "expired":
      return "Este enlace ha caducado (la cita ya ha pasado).";
    case "used":
      return "Este enlace ya se ha usado.";
    case "wrong_scope":
      return "Este enlace no es válido para cancelar.";
    case "bad_signature":
    case "malformed":
...
function htmlError(status: number, message: string): Response {
  return new Response(layout(`<h1>No se pudo cancelar</h1><p>${escape(message)}</p>`), {
    status,
    headers: { "Content-Type": "text/html; charset=utf-8" },
  });
}
...
<head>
  ...
  <style>
    ...
    h1 { font-size: 1.6rem; margin-bottom: 0.5rem; color: #b45309; }
    p { margin: 0.8rem 0; }
  </style>
</head>
<body>${body}</body>
