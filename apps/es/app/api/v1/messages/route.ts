 ...
 * GET /api/v1/messages
 *
 * Devuelve mensajes del tenant autenticado, filtros opcionales.
 * Auth: X-Tenant-Key
 *
 * Query params:
 *  - session: session_id específico
 *  - from, to: ISO dates
 *  - limit: hasta 200, default 50
 *  - offset: paginación
 */
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    tenant = await validateTenantKey(
      ...
      authContextFromRequest(request, "/api/v1/messages")
    );
  } catch (e) {
