...
import { getContactFormBackend } from "@ebecerra/sanity-client";
import type { ContactFormBackend } from "@ebecerra/sanity-client";

// Rate limit naive — 3 requests / minuto / IP en memoria.
...
// Cache module-level del schema del form. TTL 5 min.
...
async function getSchema(): Promise<ContactFormBackend | null> {
  ...
  try {
    const schema = await getContactFormBackend();
    cachedSchema = schema;
    cachedAt = now;
    return schema;
  } catch {
    // Si Sanity cae, preferimos aceptar el dato a rechazar por caída
    // externa — devolvemos null y el caller no aplica required dinámico.
    return cachedSchema;
  }
}
...
export async function POST(request: NextRequest) {
  ...
  // Validación dinámica desde Sanity (required + tipos básicos). Si el
  // schema no se puede cargar, aceptamos lo que llegue (mejor aceptar
  // dato válido que rechazar por caída de Sanity).
  ...
  // Cuerpo del email: itera answers ordenadas por prefix sN_ con humanizeKey().
  ...
  for (const k of keys) {
    ...
    // Heurísticas para subject y replyTo
  ...
  const { data, error } = await resend.emails.send(
    {
      ...
      subject: `[ebecerra.es · contacto] ${nameForSubject || "Nuevo mensaje"}`,
      text: bodyLines.join("\n"),
    },
