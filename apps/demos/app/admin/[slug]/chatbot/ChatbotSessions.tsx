...
export function ChatbotSessions({ slug }: { slug: string }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/admin/${slug}/api/chatbot/sessions?limit=100`);
  ...
  if (sessions.length === 0)
    return <div className="admin-empty">Aún no hay conversaciones.</div>;
  ...
  return (
    <div className="admin-sessions">
      {sessions.map((s) => (
        <Link
          key={s.session_id}
          href={`/admin/${slug}/chatbot/sessions/${s.session_id}`}
